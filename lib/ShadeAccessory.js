...
const SHADE_CMD_OPEN = 'open';
const SHADE_CMD_CLOSE = 'close';
const SHADE_CMD_STOP = 'stop';
...
const SHADE_DP_CONTROL = 1;
const SHADE_DP_POSITION = 2;

const POSITION_UPDATE_TIMEOUT = 60 * 1000;

class ShadeAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.dpControl = this._getCustomDP(this.device.context.dpControl) || SHADE_DP_CONTROL;
        this.dpPosition = this._getCustomDP(this.device.context.dpPosition) || SHADE_DP_POSITION;

        this.cmdOpen = SHADE_CMD_OPEN;
        if (this.device.context.cmdOpen) {
            this.cmdOpen = ('' + this.device.context.cmdOpen).trim();
        }

        this.cmdClose = SHADE_CMD_CLOSE;
        if (this.device.context.cmdClose) {
            this.cmdClose = ('' + this.device.context.cmdClose).trim();
        }

        this.cmdStop = SHADE_CMD_STOP;
        ...
        this.device.on('change', changes => {
            if (changes.hasOwnProperty(this.dpControl) && this.commandedState !== changes[this.dpControl]) {
                ...
                console.log("[TuyaAccessory] Shade " + this.device.context.name + " commanded to " + this.commandedState);
                ...
                this.positionUpdateTimeout = setTimeout(() => {
                    this.getCurrentPosition((unused, position) => {
                        ...
                        console.log("[TuyaAccessory] Shade " + this.device.context.name + " resetting position to " + position + " after timeout.");
                ...
                console.log("[TuyaAccessory] Shade " + this.device.context.name + " saw change to " + changes[this.dpPosition]);
                ...
                console.log("[TuyaAccessory] Shade " + this.device.context.name + " skipping message (likely due to recent target change): " + JSON.stringify(changes));
            }
        });
    }
    ...
    setTargetPosition(value, callback) {
        console.log('[TuyaAccessory] Shade ' + this.device.context.name + ' target position set to ' + value);
